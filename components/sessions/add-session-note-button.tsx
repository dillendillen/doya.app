...
type AddSessionNoteButtonProps = {
  sessionId: string;
  dogName: string;
  canLinkToDog: boolean;
};

export function AddSessionNoteButton({
  sessionId,
  dogName,
  canLinkToDog,
}: AddSessionNoteButtonProps) {
  const router = useRouter();
  const [isOpen, setIsOpen] = useState(false);
  const [note, setNote] = useState("");
  const [linkToDog, setLinkToDog] = useState(false);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`/api/sessions/${sessionId}`, {
        ...
        body: JSON.stringify({
          note: note.trim(),
          linkNoteToDog: canLinkToDog ? linkToDog : false,
        }),
      });
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform rounded-2xl bg-white p-6 shadow-xl transition-all">
                  ...
                  <Dialog.Description className="mt-1 text-sm text-neutral-500">
                    Capture an internal session note. You can also push it to {dogName}&apos;s profile if it should appear in their history.
                  </Dialog.Description>
                  ...
                  <form className="mt-4 space-y-4" onSubmit={handleSubmit}>
                    <div className="space-y-1">
                      ...
                      <textarea
                        ...
                        placeholder="Write a quick session recap, behaviour observation, or homework…"
                      />
                    </div>
                    ...
                    {canLinkToDog && (
                      <label className="flex items-start gap-2 text-sm text-neutral-600">
                        <input
                          type="checkbox"
                          checked={linkToDog}
                          onChange={(event) => setLinkToDog(event.target.checked)}
                          className="mt-1 h-4 w-4 rounded border-neutral-300 text-brand-primary focus:ring-brand-primary/60"
                        />
                        <span>
                          Also show on {dogName}&apos;s profile
                          <span className="block text-xs text-neutral-500">
                            Dog notes appear on both the session view and dog profile.
                          </span>
                        </span>
                      </label>
                    )}
